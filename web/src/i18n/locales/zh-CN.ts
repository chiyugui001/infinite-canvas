export default {
    ...
    dreamina: { imageModel: "即梦 Dreamina（本地生图）", videoModel: "即梦 Dreamina（本地视频）", imageModelVersion: "即梦 {{version}}（本地生图）", imageModelVersionTextOnly: "即梦 {{version}}（仅文生图）", videoModelVersion: "{{version}}（即梦本地视频）", videoModelVersionImageOnly: "{{version}}（仅图生视频）", agentRequired: "使用即梦前请先连接本地 Agent", unavailable: "本机未检测到即梦 CLI", creditConfirm: "即将提交真实即梦任务\n\n本次使用：{{usage}}\n模型：{{models}}\n参数：{{parameters}}\n当前积分余额：{{balance}}\n本次将消耗：{{credits}} 积分\n报价依据：{{quoteBasis}}\n\n确认后 Agent 会在提交前再次核验；报价变化、参数变化或报价过期都不会提交。\n确认继续吗？", webQuoteBasis: "Edge 中即梦网页当前实时显示（含本地验证规则）", historyQuoteBasis: "当前账号最近 {{evidenceCount}} 条同参数真实结算", liveCreditPerImage: "{{credits}}/张", liveCreditVideo: "{{credits}} 积分", liveCreditTotal: "共 {{credits}} 积分", liveCreditLoading: "积分查询中…", liveCreditUnavailable: "实时积分不可用", quoteUnavailable: "当前参数没有可核验的提交前积分报价，本次不会提交", creditUsageImages: "生成 {{count}} 张图片（{{tasks}} 个任务）", creditUsageVideos: "生成 {{count}} 个视频", creditUsageSeparator: "；", creditBalanceUnknown: "读取失败", noMedia: "即梦没有返回可用媒体", noVideo: "即梦没有返回视频" },
    ...
    canvas: {
        ...
        node: {
            ...
            dreamina: {
                phases: { submitting: "正在提交到即梦", submitted: "任务已提交", queued: "即梦排队中", processing: "即梦平台处理中", awaiting_confirmation: "可能等待平台确认", downloading: "正在下载并回填" },
                hints: { submitting: "请求已发送给本地 CLI，正在等待即梦返回任务 ID。", submitted: "已取得任务 ID，正在读取平台状态。", queued: "任务仍在即梦队列中。", processing: "平台正在处理，画布会自动刷新。", awaiting_confirmation: "", downloading: "结果已生成，正在写回画布。" },
                confirmationHint: "队列已经结束，但任务仍未完成；请检查即梦平台是否需要确认后继续生成。",
                cliStatus: "CLI {{status}}", queueStatus: "队列 {{status}}", queuePosition: "第 {{position}} / {{total}} 位", elapsed: "已等待 {{time}}",
            },
        },
        sidePanel: {
    ...
    agent: {
        ...
        eventExtra: { toolNamed: "调用工具：{{name}}", toolOperation: "工具操作", targetPage: "目标页面", searchContent: "搜索内容", textContent: "文本内容", operationContent: "操作内容", imageCount: "图片数量", mediaCount: "媒体数量", tools: { generateImage: "生成图片", viewImage: "查看图片", executeCommand: "执行命令", editFiles: "修改文件", searchWeb: "搜索资料", canvasOps: "画布操作", readCanvas: "读取画布", readSelection: "读取选区", exportSnapshot: "导出快照", createNode: "创建节点", addAttachments: "添加附件图片", importLocalMedia: "导入本地媒体", createText: "创建文本", createTexts: "批量创建文本", createConfig: "创建生成配置", createImageFlow: "创建生图流程", createGenerationFlow: "创建生成流程", generateText: "生成文本", generateVideo: "生成视频", generateAudio: "生成音频", updateNode: "更新节点", updateText: "更新文本", moveNodes: "移动节点", resizeNode: "调整节点尺寸", deleteNodes: "删除节点", connectNodes: "连接节点", selectNodes: "选择节点", setViewport: "调整视口", runGeneration: "触发生成", openPage: "打开页面" } },
