...
+ [新增] Canvas Agent 支持自动发现本机 Dreamina（即梦）CLI、加载其 Skill，并将 CLI 下载的图片、视频和音频导入当前画布。
+ [新增] 生图、改图、视频工作台、画布生成节点及插件宿主统一支持选择本地即梦渠道，真实任务提交前确认积分消耗，并提供完全不调用 CLI 的零积分自动测试模式。
+ [修复] 生图和视频参数面板随当前模型切换，即梦仅显示 CLI 实际支持的清晰度、比例、时长及生成数量。
+ [优化] 即梦模型选择下钻到 CLI 当前支持的具体生图与 Seedance 版本，新画布默认使用即梦 5.0 生图和 Seedance 2.0 Fast 视频。
+ [修复] 即梦任务改为持久化任务 ID 并由画布轮询结果，页面刷新或长任务超时后会自动续接并回填原图片节点，不再要求重新生成。
+ [优化] 即梦画布节点实时显示提交、排队、处理、下载阶段，以及 CLI 状态、队列位置、等待时长和可能需要平台确认的操作提示。
+ [修复] 即梦平台已接收任务但 CLI 提交响应超时时，Agent 会从最近任务中找回匹配的任务 ID 并继续回填画布，避免刷新后误报生成中断。
+ [修复] 即梦积分确认按同批真实任务汇总生成数量、模型和参数，并读取当前积分余额；移除五秒全局授权缓存，零积分测试模式仍不弹窗。
